...
static bool multiple_auth_enabled() {
	return lib->settings->get_bool(lib->settings, "%s.multiple_authentication",
			TRUE, charon->name);
}
...
static status_t collect_my_init_data(private_ike_auth_t *this,
	...
	if (this->ike_sa->generate_message(this->ike_sa, message, &this->my_packet)
			!= SUCCESS) {
		return FAILED;
	}
	return NEED_MORE;
...
static bool load_cfg_candidates(private_ike_auth_t *this) {
	...
	while (enumerator->enumerate(enumerator, &peer_cfg)) {
		...
		if (this->peer_cfg == NULL) { /* best match */
...
static bool update_cfg_candidates(private_ike_auth_t *this, bool strict) {
	do {
		if (this->peer_cfg) {
			...
			e2 = this->peer_cfg->create_auth_cfg_enumerator(this->peer_cfg,
					FALSE);

			if (strict) { /* swap lists in strict mode: all configured rounds must be
...
METHOD(task_t, build_i, status_t,
	...
	if (message->get_message_id(message) == 1) { /* in the first IKE_AUTH ... */
		if (this->ike_sa->supports_extension(this->ike_sa, EXT_MULTIPLE_AUTH)) { /* indicate support for multiple authentication */
	...
	if (!this->do_another_auth && !this->my_auth) { /* we have done our rounds */
		return NEED_MORE;
	}
	...
	if (this->my_auth == NULL) {
		...
		if (cfg) {
			...
			if (!cfg->get(cfg, AUTH_RULE_IDENTITY_LOOSE) && idr
				...
				id_payload = id_payload_create_from_identification(ID_RESPONDER,
						idr);
		...
		if (!idi || idi->get_type(idi) == ID_ANY) { /* ID_ANY is invalid as IDi, use local IP address instead */
			host_t *me;
		...
		if (idr && message->get_message_id(message) == 1
				&& this->peer_cfg->get_unique_policy(this->peer_cfg)
						!= UNIQUE_NO
				&& this->peer_cfg->get_unique_policy(this->peer_cfg)
						!= UNIQUE_NEVER) {
			host_t *host;
			...
			if (!charon->ike_sa_manager->has_contact(charon->ike_sa_manager,
				...
				message->add_notify(message, FALSE, INITIAL_CONTACT,
						chunk_empty);
			}
		}
		...
		this->my_auth = authenticator_create_builder(this->ike_sa, cfg,
				...
				this->my_packet->get_data(this->my_packet), this->reserved);
		if (!this->my_auth) {
	...
	if (do_another_auth(this)) {
		if (message->get_payload(message, AUTHENTICATION)) {
			message->add_notify(message, FALSE, ANOTHER_AUTH_FOLLOWS,
					chunk_empty);
		}
	} else {
...
METHOD(task_t, process_r, status_t,
	...
	if (message->get_message_id(message) == 1) { /* check for extensions in the first IKE_AUTH */
	...
	if (this->other_auth == NULL) {
		...
		if (message->get_payload(message, AUTHENTICATION) == NULL) { /* before authenticating with EAP, we need a EAP config */
			cand = get_auth_cfg(this, FALSE);
			while (!cand
					|| ((uintptr_t) cand->get(cand, AUTH_RULE_EAP_TYPE)
							== EAP_NAK
							&& (uintptr_t) cand->get(cand, AUTH_RULE_EAP_VENDOR)
									== 0)) { /* peer requested EAP, but current config does not match */
		...
		this->other_auth = authenticator_create_verifier(this->ike_sa, message,
				this->other_nonce, this->my_nonce,
				this->other_packet->get_data(this->other_packet),
				this->my_packet->get_data(this->my_packet), this->reserved);
		if (!this->other_auth) {
	...
	case SUCCESS:
		...
		break;
	case NEED_MORE:
		if (message->get_payload(message, AUTHENTICATION)) { /* AUTH verification successful, but another build() needed */
			break;
		}
		return NEED_MORE;
	default:
...
METHOD(task_t, build_r, status_t,
	...
	if (this->my_auth == NULL && this->do_another_auth) {
		...
		if (id->get_type(id) == ID_ANY) { /* no IDr received, apply configured ID */
			if (!id_cfg || id_cfg->contains_wildcards(id_cfg)) { /* no ID configured, use local IP address */
				host_t *me;
			...
			id = id_cfg;
		} else { /* IDr received, check if it matches configuration */
		...
		if ((uintptr_t) cfg->get(cfg, AUTH_RULE_AUTH_CLASS) == AUTH_CLASS_EAP) { /* EAP-only authentication */
	...
	if (this->other_auth) {
		...
		case NEED_MORE:
			break;
		default:
			if (message->get_payload(message, EXTENSIBLE_AUTHENTICATION)) { /* skip AUTHENTICATION_FAILED if we have EAP_FAILURE */
	...
	if (!this->do_another_auth && !this->expect_another_auth) {
		...
		DBG0(DBG_IKE, "IKE_SA %s[%d] established between %H[%Y]...%H[%Y]",
				this->ike_sa->get_name(this->ike_sa), this->ike_sa->get_unique_id(this->ike_sa), this->ike_sa->get_my_host(this->ike_sa), this->ike_sa->get_my_id(this->ike_sa), this->ike_sa->get_other_host(this->ike_sa), this->ike_sa->get_other_id(this->ike_sa));
	...
	peer_auth_failed: message->add_notify(message, TRUE, AUTHENTICATION_FAILED,
			chunk_empty);
	peer_auth_failed_no_notify: charon->bus->alert(charon->bus,
			ALERT_PEER_AUTH_FAILED);
	return FAILED;
	local_auth_failed: message->add_notify(message, TRUE, AUTHENTICATION_FAILED,
			chunk_empty);
	charon->bus->alert(charon->bus, ALERT_LOCAL_AUTH_FAILED);
	return FAILED;
}
...
METHOD(task_t, process_i, status_t,
	...
	if (!this->expect_another_auth && !this->do_another_auth
			&& !this->my_auth) {
		if (!update_cfg_candidates(this, TRUE)) {
			goto peer_auth_failed;
		}
	...
	peer_auth_failed: charon->bus->alert(charon->bus, ALERT_PEER_AUTH_FAILED);
	return FAILED;
}
...
ike_auth_t *ike_auth_create(ike_sa_t *ike_sa, bool initiator) {
	...
	INIT(this,
			.public = { .task = { .get_type = _get_type, .migrate = _migrate, .build = _build_r, .process = _process_r, .destroy = _destroy, }, }, .ike_sa = ike_sa, .initiator = initiator, .candidates = linked_list_create(), .do_another_auth = TRUE, .expect_another_auth = TRUE, );
	if (initiator) {
